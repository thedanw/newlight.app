...
| #66 (Journey Grid Settings UI) | "tabbed Tracks/Categories/Stages; drag-and-drop; delete safeguards; seeded/terminal stages have no delete action; category tree with self-parenting protection" | `JourneySettingsManager.tsx` renders a single CSS Grid (rows × stages) via `buildRows` + `Reorder.Root` (framer-motion) for vertical track reordering + `useDrag` (`@use-gesture/react`) for horizontal stage column reordering. Category nesting via flat-list hierarchy; inline add buttons for track/category/stage. `deleteJourneyStage` blocks seeded stages, `deleteJourneyTrack` requires migration target (prompt() for now, slated for UI), `saveJourneyCategory` blocks self-parenting via `validateCategory` | ✅ matches (Tabs reframed as unified grid) |
