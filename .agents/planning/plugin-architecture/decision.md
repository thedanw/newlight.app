...
**References:** `core/elvanto/{ELVANTO_SYNC_CONTRACT, ELVANTO_API_REFERENCE, ELVANTO_MIGRATION_PLAN, SYNC_PLUGIN_FIELD_MAPPING_UI, ELVANTO_SYNC_PLUGIN_RUNBOOK}.md`, `core/database/decision.md` (database architecture SSOT), `core/decision.md`, `.env` (runtime only).

**Database SSOT:** `core/database/decision.md` — table schemas, ownership, RLS, credential encryption, watermark, sync dead-letter/tombstones, seeds.
...
- **Owned tables** (`elvanto_settings`, `elvanto_sync_config`, `elvanto_sync_history`, `elvanto_sync_dead_letter`) → schema/RLS: [database/decision.md §C.7](../core/database/decision.md)
...
- **Credentials:** key encrypted in `elvanto_settings.api_key_encrypted`, never env → [database/decision.md §C.7.1](../core/database/decision.md)
- **Partial success:** edge fn returns HTTP 207 `success:false` on any entity failure → gate `!response.ok || body.success === false`.
- **Watermark upsert:** `elvanto_sync_config.key` UNIQUE-not-PK → `onConflict:'key'` required ([database/decision.md §C.7.2](../core/database/decision.md))
- **Schema traps:** `people.id` no default; `people.journey` jsonb NOT NULL + CHECK `<> '{}'` → always emit non-empty journey ([database/decision.md §B.4](../core/database/decision.md))
- **Enum sanitizers:** mapped enums wrapped (`capitalize_enum` → `Male`/`Female`; `gender` enum lowercase).
- **RLS:** authed rw settings/config; authed ro history/dead-letter; service_role bypasses RLS → [database/decision.md §A.2.4/C.7.3](../core/database/decision.md)
- **No SQL DELETE:** missing upstream ⇒ `deleted_at` tombstone + audit → [database/decision.md §C.4](../core/database/decision.md)
- **`people.journey` JSONB** = single status source — never a separate status column ([database/decision.md §B.4](../core/database/decision.md))
...
1 **Plugin system core**
    ...
    1.3 Settings hook: "Integrations" section (order 100) → surfaces in Super Admin only when enabled
        1.4 Dedicated plugin-owned tables → avoid modifying existing tables (schema → [database/decision.md §C.7](../core/database/decision.md))
2 **Sync runtime**
    ...
    2.2 Pull-first (MVP: Elvanto → Supabase) → Elvanto source of truth; write-back explicit admin action only
        2.3 Data owner = service_role → bypasses RLS + app audit → [database/decision.md §A.2.4/C.7.3](../core/database/decision.md)
        2.4 Config in `elvanto_sync_config` (JSONB key/value) → dedicated ([database/decision.md §C.7.2](../core/database/decision.md))
        2.5 Credentials in `elvanto_settings` (encrypted) → AES-GCM envelope ([database/decision.md §C.7.1](../core/database/decision.md))
        2.6 Cron + manual trigger + history + dead letter tables → admin visibility ([database/decision.md §C.7.3](../core/database/decision.md))
3 **Sync engine**
    3.1 Transforms (14 pure) + mapping-engine + watermark + entity syncs → unit-testable, config-driven
        3.2 Incremental via `date_modified` watermark → stored in `elvanto_sync_config` ([database/decision.md §C.7.2](../core/database/decision.md))
