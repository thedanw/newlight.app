...
19 Settings split shell (iOS list + detail)
   ...
   19.7 lint-pages recognizes hosted settings pages structurally — any file under a `settings/` directory must be content-only (zero `Page.*` slots), while the shell `dashboard.tsx` itself stays guarded → every future settings page is covered with no allowlist upkeep
   19.8 Card list is grouped iOS-style: **General Settings** (sections with `group: 'general'` — core platform settings) then **Modules** (default for module/plugin sections); grouping + labels live in `getSettingsSectionGroups()` (schema.ts), not the view → new sections categorize automatically
