...
- **Dashboard** (no `:section` param): the standard scaffold hosts the section card list — each card is a solid icon tile (`colorPalette.solid.bg`), title, truncated description, chevron. The icon comes from the section registration (`SettingsSection.icon`, i.e. the module's manifest icon; `SlidersHorizontal` fallback), shared with the shell headings. Cards are grouped iOS-style under muted `xs` headers: **General Settings** (core sections declaring `group: 'general'`) then **Modules** (default group for module/plugin sections) via `getSettingsSectionGroups()`.
