...
export function TagsSection({ person }: { person: Person | PersonPublic }) {
  ...
  return (
    <ProfileSection title="Tags">
      ...
      <ProfileField label="Journey tracks" value={Object.keys((person as Person).journey || {}).length ? Object.keys((person as Person).journey).join(', ') : 'No journey tracks assigned'} />
    </ProfileSection>
  )
}
