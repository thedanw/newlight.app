...
import type { DragEndEvent, DragStartEvent, SensorDescriptor } from '@dnd-kit/abstract'
...
function StageColumn({
  ...
  const { isDragging, handleRef, ref: sortableRef } = useSortable({
    id: stage.id,
    index,
    group: 'stage-columns',
    type: 'column',
  ...
  return (
    <div
     ...
     >
      {renderColumn ? (
        renderColumn(stage, { handleRef, isDragging })
      ) : (
...
export function SortableStageColumns({
  ...
  const activeStage = activeId ? stages.find((s) => String(s.id) === String(activeId)) : null
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const sourceIndex = stages.findIndex((s) => s.id === source.id)
