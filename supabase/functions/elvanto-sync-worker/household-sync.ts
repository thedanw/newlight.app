 ...
 */ export async function syncHouseholds(supabase, elvantoClient, options) {
  const result = {
    success: true,
    itemsProcessed: 0,
    itemsFailed: 0,
  ...
  try {
    ...
    const { data: people, error } = await supabase.from('people').select('id, family_id, household_id, firstname, lastname').not('family_id', 'is', null);
