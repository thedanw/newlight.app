 ...
 */ export async function syncJourney(supabase, elvantoClient, options) {
  const result = {
    success: true,
    itemsProcessed: 0,
    itemsFailed: 0,
  ...
  try {
    ...
    const { data: pairingsData } = await supabase.from('elvanto_sync_config').select('value').eq('key', 'elvanto-sync_location_track_pairings').maybeSingle();
    const pairings = pairingsData?.value ?? [];
    // Sync journey tracks based on pairings
    for (const pairing of pairings){
      try {
        // Update or create journey track
        const { error } = await supabase.from('journey_tracks').upsert({
